...
/*
 * Returns the full name for this frame.
 *
 ...
 *     class NestedClass
 *       def another_method
 *         puts caller_locations(0).map(&:full_label).join("\n")
 *       end
 *     end
 *   end
 * end
 ...
 */
static VALUE
location_full_label_m(VALUE self)
{
    return location_full_label(location_ptr(self));
}
...
Init_vm_backtrace(void)
{
    ...
    rb_define_method(rb_cBacktraceLocation, "full_label", location_full_label_m, 0);
