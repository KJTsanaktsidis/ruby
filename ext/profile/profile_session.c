...
static void
fork_helper_process(const char *helper_path, struct ProfileSession *sess)
{
    ...
    sess->helper_socket = RARRAY_AREF(sockets, 0);
    ...
    sess->helper_stderr = RARRAY_AREF(pipes, 0);
    ...
    sess->helper_pidfd = make_io_for_fd(raw_pidfd);
    sess->helper_pid = pid;
    ...
    int child_status;
    int r = waitpid(sess->helper_pid, &child_status, WNOHANG | __WCLONE);
    if (r == -1) {
        rb_sys_fail("failed to waitpid(2) helper child");
    }
    if (r != 0) {
        /* means it _did_ exit */
        VALUE errmsg = rb_funcall(sess->helper_stderr, rb_intern("read"), 0);
...
VALUE new_profile_session(void)
{
    struct ProfileSession *sess;
    VALUE obj = TypedData_Make_Struct(cProfileSession, struct ProfileSession,
                                      &profile_session_data_type, sess);
    sess->helper_pid = 0;
    sess->helper_pidfd = -1;
    sess->helper_socket = -1;

    /* Find & fork the helper process */
    VALUE helper_path = find_helper_binary_path();
    fork_helper_process(StringValueCStr(helper_path), sess);
    return obj;
}

void close_profile_session(VALUE self)
{
    struct ProfileSession *sess;
    TypedData_Get_Struct(self, struct ProfileSession,
                         &profile_session_data_type, sess);

    if (RB_TEST(sess->helper_socket)) {
        rb_funcall(sess->helper_socket, rb_intern("close"), 0);
        sess->helper_socket = Qnil;
    }
    if (RB_TEST(sess->helper_pidfd)) {
        int pidfd = RFILE(sess->helper_pidfd)->fptr->fd;
        pidfd_send_signal(pidfd, SIGKILL, NULL, 0);
        rb_funcall(sess->helper_pidfd, rb_intern("close"), 0);
        sess->helper_pidfd = Qnil;
    }
    if (RB_TEST(sess->helper_stderr)) {
        rb_funcall(sess->helper_stderr, rb_intern("close"), 0);
        sess->helper_stderr = Qnil;
    }
    if (sess->helper_pid) {
        int r = waitpid(sess->helper_pid, NULL, __WCLONE);
        if (r == -1) {
            rb_sys_fail("failed to waitpid(2) helper child");
        }
        sess->helper_pid = 0;
