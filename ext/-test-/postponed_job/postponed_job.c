...
static VALUE
pjob_register_full(VALUE self)
{
    int rfirst = rb_postponed_job_register(0, pjob_noop_callback, NULL);
    for (int i = 1; i < 2048; i++) {
        rb_postponed_job_register(0, pjob_noop_callback, NULL);
    }
    int rlast = rb_postponed_job_register(0, pjob_noop_callback, NULL);

    VALUE ary = rb_ary_new();
    rb_ary_push(ary, INT2FIX(rfirst));
    rb_ary_push(ary, INT2FIX(rlast));
    return ary;
}

static VALUE
pjob_register_one_return(VALUE self)
{
    rb_gc_start();
...
Init_postponed_job(VALUE self)
{
    ...
    rb_define_module_function(mBug, "postponed_job_register_full", pjob_register_full, 0);
    rb_define_module_function(mBug, "postponed_job_register_one_return", pjob_register_one_return, 0);
#ifdef HAVE_PTHREAD_H
